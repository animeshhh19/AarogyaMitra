...
\item We will deliver meaningful increments quickly by cloning and adapting an existing open-source reference implementation, then prototyping the core consultation workflow and validating it within a short iteration window.
...
A typical decoupled web architecture, matching the
...
already available and proven in the reference
repository, so the team focuses on workflow and safety
...
\begin{itemize}[leftmargin=0.7cm]
\item Fork/clone the reference repository and set up local + staging environments.
...
iterations on a proven codebase, implementing CI/CD to
